...
export default function InventoryPage() {
  ...
  const activeTab: TabKey = isValidTab(rawTab) ? rawTab : 'raw-materials';

  const setTab = (key: TabKey) => {
    setSearchParams(key === 'raw-materials' ? {} : { tab: key }, { replace: true });
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="font-display text-2xl font-semibold text-primary">Inventory</h1>
        <p className="mt-1 text-sm text-secondary">Raw materials, finished goods, stock levels, and adjustments</p>
      </div>
      ...
      <div>
        ...
        {activeTab === 'stock' && <StockOverviewPage />}
