...
// Extended DTO to handle fields not yet in generated client
...
export default function FinishedGoodsPage() {
  ...
  const loadBatches = async (product: ExtendedFinishedGoodDto) => {
    ...
    try {
      ...
      // batch load fails silently — panel shows empty state
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && selectedProduct?.id) {
        await updateFinishedGood(selectedProduct.id, payload, session);
      } else {
  ...
  const filteredProducts = products.filter(
    (p) =>
      ...
      (p.productCode?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false)
  );
            ...
            <div className="overflow-hidden rounded-lg border border-border bg-background">
              <Table>
                ...
                <TableBody>
                  {loading ? (
                    <TableRow>
                      <TableCell colSpan={4} className="text-center h-24 text-secondary">
                        Loading...
                      </TableCell>
                    </TableRow>
                    ...
                    filteredProducts.map((product) => (
                      <TableRow
                        ...
                        className={clsx(
                          'cursor-pointer',
                          selectedProduct?.id === product.id && 'bg-surface-highlight'
                        )}
                      >
                        <TableCell label="Product" className="font-medium text-primary">
                          {product.name}
                        </TableCell>
                        <TableCell label="Code" className="text-secondary">
                          {product.productCode}
                        </TableCell>
                        <TableCell label="Unit" className="text-secondary">
                          {product.unit}
                        </TableCell>
        ...
        <div>
          {selectedProduct ? (
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 bg-surface-highlight rounded-lg">
                      ...
                      <div className="font-medium text-lg text-primary">
                        {(selectedProduct.currentStock ?? 0).toLocaleString()}{' '}
                    ...
                    <div className="p-3 bg-surface-highlight rounded-lg">
                      ...
                      <div className="font-medium text-lg text-primary">
                        {(selectedProduct.reservedStock ?? 0).toLocaleString()}{' '}
                        {selectedProduct.unit}
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {batchesLoading ? (
                      ...
                      <p className="text-sm text-secondary text-center py-4 border border-dashed border-border rounded-lg">
                        No batches found
                      </p>
                    ) : (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-8 border border-dashed border-border rounded-xl">
              <p className="text-sm text-secondary">
                Select a product to view details and batches
              </p>
            </div>
          )}
        </div>
