...
function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const items = useMemo(
    () => [
      ...
      { label: 'Credit Requests', to: '/sales/credit-requests' },
      { label: 'Request Override', to: '/sales/credit-overrides' },
      { label: 'Dispatch Queue', to: '/sales/dispatch' },
...
export default function SalesLayout({ theme, onThemeChange }: SalesLayoutProps) {
  const navigate = useNavigate();
  ...
  const navigation = [
    { name: 'Dashboard', to: '/sales', end: true },
    { name: 'Dealers', to: '/sales/dealers' },
    { name: 'Orders', to: '/sales/orders' },
    { name: 'Promotions', to: '/sales/promotions' },
    { name: 'Invoices', to: '/sales/invoices' },
    { name: 'Targets', to: '/sales/targets' },
    { name: 'Credit Requests', to: '/sales/credit-requests' },
    { name: 'Request Override', to: '/sales/credit-overrides' },
    { name: 'Dispatch Queue', to: '/sales/dispatch' },
    { name: 'Returns', to: '/sales/returns' },
  ];
        ...
        <aside className={clsx(
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="flex h-16 items-center justify-between px-6 border-b border-border">
              <span className="text-lg font-semibold tracking-tight text-primary">Sales Portal</span>
              <button
                type="button"
        ...
        <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
          <header className={clsx('sticky top-0 z-30 flex items-center justify-between gap-3 border-b px-4 py-4 backdrop-blur-md border-border bg-surface/80')}>
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setMobileOpen(true)} className="rounded-lg p-2 text-secondary hover:bg-surface-highlight md:hidden" aria-label="Open navigation">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="rounded-lg p-2 text-secondary transition-colors hover:bg-surface-highlight"
