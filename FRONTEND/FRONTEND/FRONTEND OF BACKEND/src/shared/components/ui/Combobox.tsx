  ...
  return (
    <div ref={ref} className={twMerge('space-y-1.5 relative', className)}>
      ...
      <div className="relative">
        ...
        >
          <span className={clsx('truncate', selectedOption ? 'text-[var(--color-text-primary)]' : 'text-[var(--color-text-tertiary)]')}>
            {selectedOption ? selectedOption.label : placeholder}
          </span>
          <div className="flex items-center pl-2 shrink-0">
            {error ? <AlertCircle size={14} className="text-red-400" /> : <ChevronDown size={14} className="text-[var(--color-text-tertiary)]" />}
          </div>
        </button>
        
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-1.5 bg-[var(--color-surface-primary)] border border-[var(--color-border-default)] rounded-lg shadow-lg z-[var(--z-dropdown)] overflow-hidden animate-fade-in">
            ...
            <div className="max-h-60 overflow-y-auto p-1 no-scrollbar">
              {filteredOptions.length === 0 ? (
                <div className="px-3 py-2 text-[12px] text-[var(--color-text-tertiary)] text-center">No results found.</div>
              ) : (
                filteredOptions.map((opt) => (
                  <button
                    ...
                    className={clsx(
                      ...
                      opt.value === value ? 'bg-[var(--color-surface-secondary)] font-medium' : 'hover:bg-[var(--color-surface-secondary)] text-[var(--color-text-secondary)]'
                    )}
                  >
                    <span className="truncate">{opt.label}</span>
                    {opt.value === value && <Check size={14} className="text-[var(--color-neutral-900)] shrink-0" />}
                  </button>
                ))
              )}
            </div>
