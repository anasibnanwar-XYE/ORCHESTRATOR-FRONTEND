import { useEffect, useState, useCallback, type ReactNode, cloneElement, isValidElement, type ReactElement } from 'react';
    ...
    <div className="fixed inset-0 z-[var(--z-modal)]">
      <div
        className={clsx(
          ...
          isClosing ? 'animate-[fadeOut_200ms_ease-out_forwards]' : 'animate-[fadeIn_200ms_ease-out_forwards]',
        )}
        onClick={handleClose}
      />
        ...
        >
          {title && (
            <div className="px-4 py-3 border-b border-[var(--color-border-subtle)]">
              <p className="text-[12px] font-medium text-[var(--color-text-tertiary)] text-center">{title}</p>
            </div>
          )}
          <div className="py-1">
            {items.map((item) => (
              ...
              >
                {item.icon && isValidElement(item.icon) && cloneElement(item.icon as ReactElement<{ size?: number; className?: string }>, { size: 16, className: 'shrink-0' })}
      ...
      <div
        className={clsx(
          'sm:hidden absolute bottom-0 left-0 right-0 px-3 pb-3',
          ...
          {items.map((item, i) => (
            <button
              ...
              className={clsx(
                ...
                item.variant === 'danger' ? 'text-red-600' : 'text-[var(--color-text-primary)]',
                i < items.length - 1 && 'border-b border-[var(--color-border-subtle)]',
              )}
              style={{ WebkitTapHighlightColor: 'transparent' }}
            >
              {item.icon && isValidElement(item.icon) && cloneElement(item.icon as ReactElement<{ size?: number }>, { size: 18 })}
