      ...
      <div className="relative">
        <div
          ...
          className={clsx(
            ...
            isOpen && !error && 'ring-2 ring-[var(--color-neutral-900)]/10 border-[var(--color-neutral-400)]'
          )}
        >
        ...
        {isOpen && (
          <div className="absolute top-full left-0 right-0 mt-1.5 bg-[var(--color-surface-primary)] border border-[var(--color-border-default)] rounded-lg shadow-lg z-[var(--z-dropdown)] max-h-60 overflow-y-auto no-scrollbar py-1 animate-fade-in">
            {options.map((opt) => {
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    isSelected ? 'bg-[var(--color-surface-secondary)] font-medium text-[var(--color-text-primary)]' : 'hover:bg-[var(--color-surface-secondary)] text-[var(--color-text-secondary)]'
                  )}
                >
                  <span className="truncate">{opt.label}</span>
                  {isSelected && <Check size={14} className="text-[var(--color-neutral-900)] shrink-0" />}
                </button>
              );
            })}
          </div>
