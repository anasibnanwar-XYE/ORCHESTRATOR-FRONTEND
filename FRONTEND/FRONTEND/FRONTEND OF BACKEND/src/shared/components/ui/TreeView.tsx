...
export function TreeView({ data, defaultExpanded = false, className, onSelect }: TreeViewProps) {
              ...
              >
                <div className="flex items-center gap-1.5 min-w-0">
                  {hasChildren ? (
                    ...
                    <div className="w-[18px] shrink-0" /> // Spacer
                  )}
                  
                  {hasChildren ? (
                    <Folder size={14} className={clsx("shrink-0", isExpanded ? "text-blue-500" : "text-[var(--color-text-tertiary)]")} />
                  ) : (
  ...
  return (
    <div className={twMerge('bg-[var(--color-surface-primary)] p-2 rounded-xl border border-[var(--color-border-default)] overflow-x-auto', className)}>
      {renderTree(data)}
    </div>
  );
}
