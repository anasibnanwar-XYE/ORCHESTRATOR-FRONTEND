...
export function Accordion({ items, allowMultiple = false, className }: AccordionProps) {
  ...
  return (
    <div
      className={twMerge(
        'divide-y divide-[var(--color-border-default)] border border-[var(--color-border-default)] rounded-xl overflow-hidden bg-[var(--color-surface-primary)]',
        className,
      )}
    >
      {items.map((item) => {
        ...
        return (
          <div key={item.id}>
            ...
            >
              <div className="p-4 text-[13px] text-[var(--color-text-secondary)] border-t border-[var(--color-border-subtle)] pt-3">
                {item.content}
              </div>
            </div>
          </div>
