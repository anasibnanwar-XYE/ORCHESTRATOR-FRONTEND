          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-2 no-scrollbar">
            {column.tasks.map((task) => (
              <div
                ...
                className="bg-[var(--color-surface-primary)] p-3 rounded-lg border border-[var(--color-border-default)] shadow-sm hover:border-[var(--color-border-strong)] cursor-grab active:cursor-grabbing transition-colors"
              >
                ...
                {task.assignee && (
                  <div className="mt-3 flex justify-end">{task.assignee}</div>
                )}
              </div>
            ))}
